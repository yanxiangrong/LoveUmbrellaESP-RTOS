...
#define I2C_MASTER_HALF_CYCLE 500
...
i2c_master_read_bytes(uint8_t dev_id, uint8_t reg_addr, uint8_t *reg_data, uint16_t len) {
    int8_t ret = I2C_OK;
    uint16_t lastread = len - 1;

    i2c_master_start();
    i2c_master_writeByte(dev_id << 1);
    if (!i2c_master_checkAck()) {
        ret = I2C_SET_DEV_FAILED;
        goto end;
    }

    i2c_master_writeByte(reg_addr);
    if (!i2c_master_checkAck()) {
        ret = I2C_SET_REGISTER_FAILED;
        goto end;
    }
    i2c_master_stop();
    ...
    // start again
    i2c_master_start();
    i2c_master_writeByte((dev_id << 1) | 1); // read
    if (!i2c_master_checkAck()) {
        ret = I2C_START_AGAIN_FAILED;
        goto end;
    }

    for (uint16_t i = 0U; i < len; i++) {
        reg_data[i] = i2c_master_readByte();
        i2c_master_setAck(i == lastread ? 1 : 0);
    }

    end:
    i2c_master_stop();
//    os_printf("i2c read returned %d. Red %d of %d\r\n", ret, i, len);
    return ret;
}
 ...
 *******************************************************************************/
int8_t ICACHE_FLASH_ATTR
i2c_master_write_bytes(uint8_t dev_id, uint8_t reg_addr, uint8_t *reg_data, uint16_t len) {
    int8_t ret = I2C_OK;
    ...
    for (uint16_t i = 0U; i < len; i++) {
        i2c_master_writeByte(reg_data[i]);
        if (!i2c_master_checkAck()) {
            ret = I2C_WRITE_BYTE_FAILED;
            goto end;
        }
    }
    ...
    i2c_master_stop();
//    os_printf("i2c write returned %d\r\n", ret);
    return ret;
}
